...
class BoshyAgent:
    ...
    def store_transition(self, state, action, reward, state_, done):
        ...
        self.downsampled_state_memory[index] = np.array((int(state[0] * self.x_grid),
                                                         int(state[1] * self.y_grid)), dtype=np.int32)
    ...
    def learn(self, verbose=False):
        ...
        if self.graph:
            self.graph_net(64 ** 2 * self.Q_eval.input_dims)
    ...
    def graph_net(self, weights_lcm):
        ...
        for layer in [self.Q_eval.fc1, self.Q_eval.fc2, self.Q_eval.fc3, self.Q_eval.fc4, self.Q_eval.fc5,
                      self.Q_eval.fc6, self.Q_eval.fc7]:
